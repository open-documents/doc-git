...
# Default list of JAR files that should not be scanned using the JarScanner functionality.
# This is typically used to scan JARs for configuration information.
# JARs that do not contain such information may be excluded from the scan to speed up the scanning process.
# This is the default list.JARs on this list are excluded from all scans. The list must be a comma separated list  
# of JAR file names.  
...
由 `org.apache.catalina.startup.CatalinaProperties` 加载。
...
1）通过System指定属性 `catalina.config` 。可以直接指定URL，也可以指定一个catalinaBase/conf/里面的一个配置文件名称。
2）`catalinaBase/conf下的 catalina.properties`。
...
if (configUrl != null) {  
    if (configUrl.indexOf('/') == -1) {  
        // No '/'.
        // Must be a file name rather than a URL  
        fileName = configUrl;  
    } else {
